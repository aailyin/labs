...
UPDATE JOB SET MINSALARY = MINSALARY * (WHEN JOBNAME = 'CLERK' THEN 1.1 ELSE 1.2 END) WHERE JOBNAME IN ('FINANCIAL DIRECTOR', 'CLERK');
...
--8.Исправьте даты рождения в таблице EMP, в которых год приходится на первый век нашей эры по следующему правилу: даты до 03 года 
...
--10.Добавьте нового сотрудника в таблицу EMP. Его имя и фамилия должны совпадать с Вашими, записанными латинскими буквами согласно паспорту, 
...
DELETE FROM tmp_emp; 
INSERT INTO tmp_emp SELECT empno, empname, birthdate FROM emp E JOIN career C ON E.empno = C.empno JOIN jobno J ON C.jobno = J.jobno WHERE J.jobname = 'CLERCK' AND C.enddate IS NULL;   
...
INSERT INTO tmp_emp SELECT empno, empname, birthdate FROM emp E JOIN career C ON E.empno = C.empno WHERE C.startdate IS NULL;
...
--16.Начислите зарплату в размере 120% минимального должностного оклада всем сотрудникам, работающим на предприятии. Зарплату начислять по должности, 
--   занимаемой сотрудником в настоящий момент и отнести ее на прошлый месяц относительно текущей даты.
INSERT INTO salary SELECT empno, EXTRACT(month from add_months(sysdate, -1)), EXTRACT(year from add_months(sysdate, -1)), 1.2 * minsalary
FROM CAREER NATURAL JOIN EMP NATURAL JOIN JOB WHERE ENDDATE IS NULL;
...
--19.Удалите информацию о начисленной зарплате сотрудников, которые в настоящий момент уже не работают на предприятии (можно использовать 
